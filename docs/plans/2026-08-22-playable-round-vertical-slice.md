...
| ----------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. Round controller           | Superseded.                                          | `lib/game/round/round_controller.dart` is absent, and [MatchController](../../lib/game/match/match_controller.dart) plus [its tests](../../test/game/match/match_controller_test.dart) own the surviving controller contract.                                                                                                                     |
| 2. Board rendering            | Implemented under the later match design.            | [RoundBoard](../../lib/game/view/round_board.dart) and [board tests](../../test/game/view/round_board_test.dart) provide the artifact and focused coverage.                                                                                                                                                                                       |
| 3. Direct game screen         | Superseded.                                          | [StartPage](../../lib/game/start/start_page.dart) now opens [GamePage](../../lib/game/view/game_page.dart), and [the app test](../../test/app/view/app_test.dart) covers that flow and Korean system-locale strings.                                                                                                                              |
| 4. Template cleanup           | Superseded in part.                                  | The obsolete loading, title, cubit, and Unicorn paths are absent. The old no-audio and Google Fonts targets are superseded by [intentional round feedback](../../lib/game/feedback/round_feedback.dart) and [bundled fonts and current dependencies](../../pubspec.yaml). This does not establish the original deletion or regeneration sequence. |
| 5. Native parity              | Implemented; current verification recorded.          | [PR #82](https://github.com/AndrewDongminYoo/ttush_push/pull/82) recorded three Android and three iOS parity tests on `e369835` before merge `6da4099`, with CI passing.                                                                                                                                                                          |
| 6. Source gate and human play | Implemented source gate; human verification pending. | PR #82 records the source gate. The external-play milestone [#42](https://github.com/AndrewDongminYoo/ttush_push/issues/42) remains in [tracker #81](https://github.com/AndrewDongminYoo/ttush_push/issues/81).                                                                                                                                   |

The original unchecked boxes stay unchanged because the [checkbox-drift index](../notes/2026-08-31-plan-checkbox-drift.md) records that this plan was not executed as written.
That is a historical provenance gap, not a request to reconstruct old TDD commands or commit execution.
The original Web exclusion is superseded by [the current Web bridge configuration](../../flutter_rust_bridge.yaml) and [Web CI job](../../.github/workflows/main.yaml), while the shipping decision remains open in tracker #81.
