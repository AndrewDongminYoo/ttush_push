...
5. Discard the previous active match, score, selected piece, opponent selection, error, retry action, and pending move. The fresh controller starts with its existing defaults. [L3]
...
4. Run the focused `game_page_test.dart`, Flutter analysis, and `merry run check`.
