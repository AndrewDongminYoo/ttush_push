...
3. **The release build actually starts and reaches Rust.** Minification runs only in the release build, so a debug run proves nothing about it, and `android/app/proguard-rules.pro` deliberately carries no rules, which leaves R8 running on the defaults plus what each dependency contributes. Since the AGP 9.1.0 upgrade of 2026-09-20 the default is `proguard-android-optimize.txt`, because AGP 9 refuses the non-optimizing file; the run below was repeated that day on the `Pixel_10` arm64 emulator with the same result. Install the release APK on any arm64 runtime and look at the first screen.
